    ...
    try {
      ...
      if (!req.headers['authorization']) {
        ...
        return res
          ...
          .send(APIResponse.withoutData(HttpStatus.UNAUTHORIZED,'Missing authorization headers'),
          );
      }
      ...
      try {
        ...
        if (!Constants.VALID_USER_TYPES.includes(userPayload.customer_type)) {
          logger.warn("You don't have the valid authorization for this resource.");
          return res
            .status(HttpStatus.FORBIDDEN)
            .send(APIResponse.withoutData(HttpStatus.FORBIDDEN,"You don't have the valid authorization for this resource."),
            );
        }

        const shouldNotSaveOrUpdateCustomer = ['/api/v1/customers'].includes(req.url) && `${req.method}`.toUpperCase() === 'PUT';
        ...
        if (!existingCustomer && !shouldNotSaveOrUpdateCustomer) {
        ...
        } else if (existingCustomer && !shouldNotSaveOrUpdateCustomer) {
