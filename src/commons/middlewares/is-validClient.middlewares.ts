...
@Injectable()
export class IsValidClientMiddleware implements NestMiddleware{
    ...
    async use(req: any, res: Response, next: NextFunction) {
        ...
        // * checking if the request came with a serviceCode
        if (!req.headers['serviceCode']) {
            return res.status(HttpStatus.UNAUTHORIZED).send({
                message: 'No service code was submitted',
            });
        }
        ...
        // if the request came with a key, serviceCode, IP address and secret, check if they are correct
        ...
        // check if service Code is correct
        if (!existingClient.serviceId) {
            logger.warn(`Api service is not found with the given code: ${existingClient.serviceId}`);
            return res.status(HttpStatus.UNAUTHORIZED).send({
                message: 'Service code is invalid',
            });
        }

         // check if ip address is correct
        if (!existingClient.ipWhitelist) {
            logger.warn(`Client IP is not found : ${existingClient.ipWhitelist}`);
            return res.status(HttpStatus.UNAUTHORIZED).send({
                message: 'Service code is invalid',
            });
        }
